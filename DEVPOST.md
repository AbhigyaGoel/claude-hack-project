...
6. **PT Supervision via MCP** — A human PT can connect via Claude Desktop and use 6 MCP tools to review patients, inspect form events, and override exercise plans — keeping a human in the loop.
...
- **Zero server-side CV.** All pose estimation runs in the browser via WASM. The server never sees raw video. This is both a privacy win and an architecture win — no GPU servers needed.
