...
import type { ExerciseCategory, DifficultyTier, BodyRegion } from "@/types/exercise";
...
export async function POST(req: NextRequest) {
  ...
  const profile = patientRows[0].profile_json as { diagnostic: Record<string, unknown> };
  const diagnostic = profile.diagnostic as {
  ...
  // Persist plan (id is generated by Postgres default)
  const [row] = await db
    .insert(plans)
    .values({
      patient_id,
  ...
  return NextResponse.json({ plan_id: row.id, plan });
}
