...
export default function ReportPage() {
  ...
  const sendChatMessage = async () => {
    ...
    try {
      const res = await fetch("/api/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ patient_id: sessionId, message: trimmed }),
      });
      if (!res.ok) throw new Error("failed");
      const data = await res.json() as { response?: string };
      setChatMessages((prev) => [...prev, { role: "assistant", content: data.response ?? "I couldn't process that." }]);
    } catch {
  ...
  const handleCopy = async () => {
    if (!report?.export_summary) return;
    await navigator.clipboard.writeText(report.export_summary).catch(() => {});
    setCopied(true);
      ...
      <header className="glass-card-bright mx-4 mt-4 p-6 sm:mx-auto sm:max-w-4xl animate-fade-in print:shadow-none">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-4 flex-wrap shrink-0">
            {hasPain && <PainDelta pre={report.pain_pre!} post={report.pain_post!} />}
            {report.overall_score != null && <ScoreRing score={report.overall_score} />}
          </div>
        </div>

        {/* Export actions */}
        <div className="flex gap-2 mt-5 pt-4 border-t border-[var(--color-border)] print:hidden">
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium transition-all duration-200"
            style={{
              background: copied ? "var(--color-success-dim)" : "var(--color-surface-raised)",
