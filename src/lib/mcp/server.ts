...
export async function handleMCPToolCall(
  ...
  switch (toolName) {
    ...
    case "get_patient_summary": {
      ...
      return {
        patient: JSON.parse(patientRows[0].profile_json),
        memory_path: patientRows[0].memory_path,
        created_at: patientRows[0].created_at,
        active_plan: activePlan[0] ? JSON.parse(activePlan[0].plan_json) : null,
    ...
    case "override_plan": {
      ...
      for (const p of existingPlans) {
        await db.update(plans).set({ active: false }).where(eq(plans.id, p.id));
      }

      // Insert new plan
      const newPlanId = `plan_override_${Date.now()}`;
      await db.insert(plans).values({
        id: newPlanId,
        patient_id: patientId,
        plan_json: planJson,
        active: true,
        citations_json: JSON.stringify({ override_reason: reason }),
        created_at: new Date().toISOString(),
      });

      return { plan_id: newPlanId, status: "Plan overridden", reason };
    }
